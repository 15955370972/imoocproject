<template>
  <scroll :probeType="probeType" class="listview" :data="data" ref="listview" :listenScroll="listenScroll" @scroll="scroll">
    <ul>
    ...
    <div class="list-shortcut" @touchstart="onShortcutTouchStart" @touchmove.stop.prevent="onShortcutTouchMove">
      <ul>
        <li :class="{'current': currentIndex===index}" v-for="(item,index) in shortcutList" :key="item" class="item" :data-index="index">
          {{item}}
        </li>
      </ul>
    </div>
    ...
    scrollY(newY) {
      ...
      for (let i = 0; i < listHeight.length - 1; i++) {
        ...
        if (-newY >= height1 && -newY < height2) {
          ...
          //diff 是用来计算当前滚动的 scrollY 值落在某个列表区块中离顶部的距离，然后基于它去设置 fixedTitle 偏移
